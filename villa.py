"""
Himachal Pradesh Residential AVM Pipeline — v3
Now trained on `hp_unified_listings.json`: 694 real scraped (99acres) listings
merged with 2,996 Avacasa internal records (ALL flagged is_synthetic=True --
generated from project seeds, not observed transactions; see caveat below).

Split: 70% train / 15% test / 15% unseen holdout (stratified by data_source
so the synthetic/real ratio is preserved across all three splits).

What this dataset already fixes vs. the old 99acres-only scrape:
  - `saleable_area_sqft` is a clean, separate field from `plot_area_sqft` --
    the mega-estate "plot area mislabeled as built area" bug from v1/v2 is
    structurally far less present here (only 1 extreme outlier in 3,690 rows).
  - `district` / `major_market` / `micro_market` give an explicit location
    hierarchy instead of needing regex-mined city parsing.
  - `furnishing`, `power_backup`, `facing`, `construction_status`,
    `parking_count` arrive as clean structured fields for the scraped rows
    instead of needing regex extraction from concatenated blobs.

CRITICAL CAVEAT (read before trusting these numbers):
  81% of this dataset (2,996 / 3,690 rows) is `data_source=avacasa_internal`
  and 100% of those rows have `is_synthetic=True` -- i.e. generated from
  project seeds, not real observed sales. Most "enrichment" fields
  (benchmark_psf, market_maturity, drive times, risk scores, elevation, etc.)
  exist ONLY for these synthetic rows and are null for every real scraped
  listing -- so a model trained on the full mix can partly learn to
  recognize "is this a synthetic Avacasa record" through those fields'
  very presence, not genuine price signal. To stay honest, this script
  reports R2/MAE for the unseen holdout BOTH overall AND split by
  data_source, so you can see whether performance is being carried by the
  synthetic majority. Treat the "scraped_99acres only" holdout numbers as
  the more trustworthy real-world estimate.
"""
...
# ----------------------------------------------------------------------
df = df[df["asset_class"] == "villa"].copy()
...
ax.legend()
plt.tight_layout()
plt.savefig("./actual_vs_predicted_villa.png", dpi=150)
plt.close()
...
plt.tight_layout()
plt.savefig("./feature_importance_villa.png", dpi=150)
plt.close()
